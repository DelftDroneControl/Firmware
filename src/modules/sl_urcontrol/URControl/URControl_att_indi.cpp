  ...
  *rtu_input_zTarget, real_T rty_wRotorSet[4])
{
  ...
  /* :  if URControlParams.DRF_enable == 0 */
  if (URControlParams.DRF_enable == 0.0) {
    /* :  if u == 1 */
  ...
  /* 'pseudo_control_att_indi:8' r = state.omegaf(3) * URControlParams.r_scale; */
  r = rtu_state_omegaf[2] * URControlParams.r_scale;

  /* 'pseudo_control_att_indi:10' Vz = state.vel(3); */
  /* 'pseudo_control_att_indi:11' Z = state.pos(3); */
  /* 'pseudo_control_att_indi:13' nx = nB(1); */
  /* 'pseudo_control_att_indi:14' ny = nB(2); */
  /* 'pseudo_control_att_indi:16' chi = URControlParams.chi/57.3; */
  a21 = URControlParams.chi / 57.3;
  chi = a21;

  /* 'pseudo_control_att_indi:18' if state.fail_id == 1 || state.fail_id == 3 */
  if ((*rtu_state_fail_id == 1.0) || (*rtu_state_fail_id == 3.0)) {
    /* 'pseudo_control_att_indi:19' chi = pi- URControlParams.chi/57.3; */
    chi = 3.1415926535897931 - a21;
  }

  /* 'pseudo_control_att_indi:22' m   = URControlParams.mass; */
  /* 'pseudo_control_att_indi:23' g   = URControlParams.g; */
  /* 'pseudo_control_att_indi:25' R_IB = [cos(psi)*cos(theta) , cos(psi)*sin(theta)*sin(phi)-sin(psi)*cos(phi), cos(psi)*sin(theta)*cos(phi)+sin(psi)*sin(phi); */
  /* 'pseudo_control_att_indi:26'         sin(psi)*cos(theta) , sin(psi)*sin(theta)*sin(phi)+cos(psi)*cos(phi), sin(psi)*sin(theta)*cos(phi)-cos(psi)*sin(phi); */
  /* 'pseudo_control_att_indi:27'         -sin(theta)          , cos(theta)*sin(phi)                          , cos(theta)*cos(phi)                          ]; */
  nu_idx_2 = std::sin(rtu_state_att[1]);
  R_IB_tmp_tmp = std::cos(rtu_state_att[2]);
  rtb_w_idx_1 = std::cos(rtu_state_att[1]);
  R_IB_tmp[0] = R_IB_tmp_tmp * rtb_w_idx_1;
  R_IB_tmp_tmp_0 = R_IB_tmp_tmp * nu_idx_2;
  rtb_w_idx_2 = std::cos(rtu_state_att[0]);
  h_idx_1 = std::sin(rtu_state_att[2]);
  h_idx_2 = std::sin(rtu_state_att[0]);
  R_IB_tmp[3] = R_IB_tmp_tmp_0 * h_idx_2 - h_idx_1 * rtb_w_idx_2;
  R_IB_tmp[6] = R_IB_tmp_tmp_0 * rtb_w_idx_2 + h_idx_1 * h_idx_2;
  R_IB_tmp[1] = h_idx_1 * rtb_w_idx_1;
  R_IB_tmp_tmp_0 = h_idx_1 * nu_idx_2;
  R_IB_tmp[4] = R_IB_tmp_tmp_0 * h_idx_2 + R_IB_tmp_tmp * rtb_w_idx_2;
  R_IB_tmp[7] = R_IB_tmp_tmp_0 * rtb_w_idx_2 - R_IB_tmp_tmp * h_idx_2;
  R_IB_tmp[2] = -nu_idx_2;
  R_IB_tmp[5] = rtb_w_idx_1 * h_idx_2;
  R_IB_tmp[8] = rtb_w_idx_1 * rtb_w_idx_2;
  memcpy(&R_IB[0], &R_IB_tmp[0], 9U * sizeof(real_T));

  /* 'pseudo_control_att_indi:28' h = R_IB\n_des; */
  r1 = 0;
  r2 = 1;
  r3 = 2;
  rtb_nd_sat_idx_1 = std::abs(R_IB_tmp[0]);
  maxval = rtb_nd_sat_idx_1;
  a21_tmp = std::abs(R_IB_tmp[1]);
  if (a21_tmp > rtb_nd_sat_idx_1) {
    maxval = a21_tmp;
    r1 = 1;
    r2 = 0;
  }
  ...
  /* 'pseudo_control_att_indi:29' h1 = h(1); */
  /* 'pseudo_control_att_indi:29' h2 = h(2); */
  /* 'pseudo_control_att_indi:37' Y = [h1-nx; h2-ny]; */
  /* 'pseudo_control_att_indi:38' dY = [-h(3)*q + h(2)*r + lambda(1); */
  /* 'pseudo_control_att_indi:39'        h(3)*p - h(1)*r + lambda(2)]; */
  /* 'pseudo_control_att_indi:41' Y = [cos(chi) sin(chi); -sin(chi) cos(chi)] * Y; */
  ...
  /* 'pseudo_control_att_indi:42' dY = [cos(chi) sin(chi); -sin(chi) cos(chi)] * dY; */
  if (u0 > 0.5) {
    u0 = 0.5;
  } else {
    if (u0 < -0.5) {
   ...
   *  BusCreator: '<S1>/BusConversion_InsertedFor_pseudo_control_indi_at_inport_0'
   */
  u0 += -h_idx_2 * rtu_state_omegaf[1] + h_idx_1 * r;
   ...
   *  UnitDelay: '<S3>/UD'
   */
  h_idx_1 = (h_idx_2 * rtu_state_omegaf[0] - h_idx_0 * r) + rtb_dY_idx_0;
  rtb_dY_idx_0 = R_IB_tmp_tmp_0 * u0 + R_IB_tmp_tmp * h_idx_1;
  u0 = -std::sin(chi) * u0 + R_IB_tmp_tmp_0 * h_idx_1;

  /* 'pseudo_control_att_indi:44' kpz = URControlParams.pos_z_p_gain; */
  /* 'pseudo_control_att_indi:45' kdz = URControlParams.pos_z_d_gain; */
  /* 'pseudo_control_att_indi:46' kp = URControlParams.att_p_gain; */
  /* 'pseudo_control_att_indi:47' kd = URControlParams.att_d_gain; */
  /* 'pseudo_control_att_indi:48' kpr = URControlParams.YRC_Kp_r; */
  /* 'pseudo_control_att_indi:50' if URControlParams.manual_enable == 0 || URControlParams.manual_alt == 0 */
  if ((URControlParams.manual_enable == 0.0) || (URControlParams.manual_alt ==
       0.0)) {
    /* 'pseudo_control_att_indi:51' nu1 =  - kdz*(Vz-Vz_ref) - kpz*(Z-Z_ref); */
    nu1 = (rtu_state_vel[2] - (rtb_TSamp_h - URControl_att_indi_DW.UD_DSTATE_c))
      * -URControlParams.pos_z_d_gain - (rtu_state_pos[2] - *rtu_input_zTarget) *
      URControlParams.pos_z_p_gain;
  } else {
    /* 'pseudo_control_att_indi:52' else */
    /* 'pseudo_control_att_indi:53' nu1 = -F_tot_ref/m * cos(theta)*cos(phi) + g; */
    nu1 = -*rtu_Ftot_ref / URControlParams.mass * rtb_w_idx_1 * rtb_w_idx_2 +
      URControlParams.g;
  }

  /* DiscreteIntegrator: '<S20>/Integrator' */
  /* 'pseudo_control_att_indi:56' nu2 =  - kd * dY(1) - kp*Y(1); */
  /* 'pseudo_control_att_indi:57' nu3 =  - kd * dY(2) - kp*Y(2); */
  /* 'pseudo_control_att_indi:58' nu4 =  - kpr* (r-r_ref); */
  /* 'pseudo_control_att_indi:60' nu = [nu1; nu2; nu3; nu4]; */
  ...
  nu_idx_3 = (r - *rtu_r_cmd) * -URControlParams.YRC_Kp_r;
  ...
  r = std::sqrt(URControlParams.b * URControlParams.b + URControlParams.l *
                URControlParams.l);
  ...
  maxval = std::atan(URControlParams.b / URControlParams.l);
  ...
  a21 = -URControlParams.k0 / URControlParams.mass * rtb_w_idx_1 * rtb_w_idx_2;
  ...
  a21_0[0] = a21;
  a21_tmp = URControlParams.k0 * r;
  chi = std::cos(maxval);
  a21_tmp_0 = a21_tmp * chi / URControlParams.Iy *
    -URControl_att_indi_DW.Integrator_DSTATE_i[2];
  a21_0[1] = a21_tmp_0 + URControl_att_indi_DW.Integrator_DSTATE_i[1] * Gr_tmp;
  maxval = std::sin(maxval);
  a21_tmp = a21_tmp * maxval / URControlParams.Ix *
    URControl_att_indi_DW.Integrator_DSTATE_i[2];
  a21_0[2] = a21_tmp - URControl_att_indi_DW.Integrator_DSTATE_i[0] * Gr_tmp;
  a21_0[3] = Gr_tmp;
  a21_0[4] = a21;
  a21_0[5] = a21_tmp_0 + URControl_att_indi_DW.Integrator_DSTATE_i[1] * Gr_tmp_0;
  a21_tmp_0 = -URControlParams.k0 * r;
  maxval = a21_tmp_0 * maxval / URControlParams.Ix *
    URControl_att_indi_DW.Integrator_DSTATE_i[2];
  a21_0[6] = maxval - URControl_att_indi_DW.Integrator_DSTATE_i[0] * Gr_tmp_0;
  a21_0[7] = Gr_tmp_0;
  a21_0[8] = a21;
  a21_tmp_0 = a21_tmp_0 * chi / URControlParams.Iy *
    -URControl_att_indi_DW.Integrator_DSTATE_i[2];
  a21_0[9] = a21_tmp_0 + URControl_att_indi_DW.Integrator_DSTATE_i[1] * Gr_tmp;
  a21_0[10] = maxval - URControl_att_indi_DW.Integrator_DSTATE_i[0] * Gr_tmp;
  a21_0[11] = Gr_tmp;
  a21_0[12] = a21;
  a21_0[13] = a21_tmp_0 + URControl_att_indi_DW.Integrator_DSTATE_i[1] *
    Gr_tmp_0;
  a21_0[14] = a21_tmp - URControl_att_indi_DW.Integrator_DSTATE_i[0] * Gr_tmp_0;
  a21_0[15] = Gr_tmp_0;
  for (r3 = 0; r3 < 4; r3++) {
    for (fail_id_size_idx_1 = 0; fail_id_size_idx_1 < 4; fail_id_size_idx_1++) {
      ...
      G[rtemp] = 0.0;
      G[rtemp] += a21_0[r1] * R[fail_id_size_idx_1];
      G[rtemp] += a21_0[r1 + 1] * R[fail_id_size_idx_1 + 4];
      G[rtemp] += a21_0[r1 + 2] * R[fail_id_size_idx_1 + 8];
      G[rtemp] += a21_0[r1 + 3] * R[fail_id_size_idx_1 + 12];
    }
  }
  ...
  r = nu1 - (URControl_att_indi_DW.Integrator_DSTATE_pv * rtb_w_idx_1 *
             rtb_w_idx_2 + 9.8124);
  rtb_w_idx_1 = (-URControlParams.att_d_gain * rtb_dY_idx_0 -
  ...
  for (r3 = 0; r3 < 4; r3++) {
    Gr[r3] = (((R[r3 + 4] * rtb_w_idx_1 + R[r3] * r) + R[r3 + 8] * nu_idx_2) +
              R[r3 + 12] * nu_idx_3) +
  ...
  rtb_w_idx_1 = std::sqrt(std::abs(Gr[1])) * chi;
  ...
  rtb_w_idx_2 = std::sqrt(std::abs(Gr[2])) * chi;
  ...
  r = static_cast<real_T>((chi == 0.0)) * 2.2204460492503131e-16 + chi;
  ...
  URControl_att_indi_DW.Integrator_DSTATE_pv += 1.0 / r * (rtu_state_acc[2] -
  ...
  URControl_att_indi_DW.Integrator_DSTATE_k += 1.0 / a21 * (rtu_state_omegaf[2]
